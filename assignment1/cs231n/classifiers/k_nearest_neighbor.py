...
class KNearestNeighbor(object):
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in xrange(num_test):
      for j in xrange(num_train):
        ...
        #####################################################################
        distance=np.sqrt(np.sum(np.power(X[i]-self.X_train[j],2)))
        dists[i,j]=distance
        #####################################################################
        #                       END OF YOUR CODE                            #
        #####################################################################
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    #将L2公式拆开计算，即（x - y）^2 = x^2 - 2*x*y + y^2
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      idx=np.argsort(dists[i,:])#idx表示第i张图与train中每张图的相似度，升序排序，返回下标
      kidx=idx[:k]#取前k个的下标
      ...
      # label.                                                                #
      #########################################################################
      l=np.bincount(closest_y)#bincount统计x中各个元素（非负整数）出现的次数，返回次数列表。下标i的值表示i在x中出现的次数
      y_pred[i]=np.argmax(l)
      #########################################################################
      #                           END OF YOUR CODE                            # 
      #########################################################################
